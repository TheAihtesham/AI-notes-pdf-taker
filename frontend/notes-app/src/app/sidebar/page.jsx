...
const GetPdfList = ({refreshTrigger}) => {
  ...
  return (
    <div className="w-full mt-6 p-4 sm:p-6 bg-white rounded-xl  space-y-6">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {pdfList.length > 0 ? (
            pdfList.map((pdf) => (
              <li
                key={pdf._id}
                className="bg-gray-50 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 p-8 flex items-center gap-4 group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <Link
                    ...
                    title={pdf.filename} // Shows full name on hover
                  >
